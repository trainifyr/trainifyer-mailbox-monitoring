...
export default function StudentAttendancePage() {
  ...
  // Lock body scroll when the session detail modal is open
  useEffect(() => {
    if (selectedSession) {
        ...
        >
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', margin: '0 0 1.5rem 0' }}>
              <div>
                <h3 style={{ fontSize: '1.25rem', margin: '0 0 0.25rem 0' }}>{selectedSession.meeting_title}</h3>
                <p style={{ fontSize: '0.8125rem', color: 'var(--text-muted)', margin: 0 }}>
                  {new Date(selectedSession.session_date).toLocaleDateString('en-GB', { day: '2-digit', month: 'long', year: 'numeric' })}
                </p>
              </div>
              <button className="btn btn-ghost" style={{ padding: '0.5rem', flexShrink: 0 }} onClick={() => setSelectedSession(null)}>
                <X size={20} />
              </button>
            </div>
